...
function dniLetter( dni ) {
	...
	inv = expReg.test(String(dni));

	if (dni == -1 || inv){
		result = "Invalid parameter";
	}
	else{
		result = letters[dni % 23 ];
	}
...
function findLongestWord( array_words ){
	...
	for (var i = 0; i < array_words.length; i++){
		aux_length = array_words[i].length;
...
function arrangeElements( array )
{
	...
	//isNaN(value) --> devolvera true si no es numero y false si es numero
	var numbers = [];
	var letters = [];
...
function beautifyLetters( array )
{
	...
	//var m = str.match(/[aeiou]/gi); --> para encontrar si es vocal
	var aux_array = [];

	for (var i in array){
		if (isNaN(array[i])){
			if(array[i].match(/[aeiou]/gi)){
				vowel = array[i].toUpperCase();
				aux_array.push(vowel);
			}else{
				consonant = array[i].toLowerCase();
				aux_array.push(consonant);
			}
